...
def update_library():
    ...
    library_folder = plugin.get_setting(SETTING_MUSIC_LIBRARY_FOLDER)
...
def add_music_to_library(library_folder, artist_name, album_name, track_name):
    ...
    if "album" in track_info:
        ...
        if track_number != "" and track_number != None: full_track_name = track_number + ". " + (track_name).encode('utf-8')
        else: full_track_name = track_name
    else: full_track_name = track_name
    ...
    if not xbmcvfs.exists(strm_filepath):
        ...
        if "album" in track_info:
            track_number = track_info["album"]["@attr"]["position"]
            strm_filepath = os.path.join(album_folder, track_number + ". " + track_name + ".strm")
